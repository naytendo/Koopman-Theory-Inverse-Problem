...
nu = 1/2*(b+dim/2);
targetScale = 1/3;
[interps,coefs] = getTargetFunctionNew(rho1,rho2,targetType,targetScale,nu-dim/2);
...
delta = [0.2 0.15 0.1 0.08 0.05 0.025];
...
for dd = 1:length(delta)
    ...
    for nn = 1:length(x_Eval)
        check = 0;
        for jj = 1:N
            if norm(centers(jj,:)-x_Eval(nn,:)) > 2*separation
                check = check +1;
            end
        end
        if check == N
            centers(N+1,:) = x_Eval(nn,:);
            N = N+1;
        end
    end
...
%%
figure(1)
loglog(delta,1000*delta.^4,'k--')
loglog(delta,1500*delta.^3.5,'k.-')
xlabel('$h_{\Xi_n,M}$','interpreter','latex')
ylabel('$Relative Error$ error','interpreter','latex')
legend(sprintf('$h_Q = %.2f$',hQ(1)),sprintf('$h_Q = %.2f$',hQ(2)),sprintf('$h_Q = %.2f$',hQ(3)),sprintf('$h_Q = %.2f$',hQ(4)),'$h^{4}_{\Xi_n,M}$','$h^{3.5}_{\Xi_n,M}$','interpreter','latex')
set(gca,'fontsize',20)
set(gca,'XScale','log')
set(gca,'YScale','log')
