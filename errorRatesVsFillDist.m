...
%     end
    ...
    kernVector = zeros(length(interps),1);
        for cc = 1:length(interps)
            kernVector(cc) = kernel(targetType,interps(cc,:),xSamp(1,:),targetHyper);
        end
    target_Func_Orb(M) = coefs'*kernVector;
    separation = 2*delta(dd);
    for mm = 1:length(xSamp)
        ...
        if check == M
            ...
            kernVector = zeros(length(interps),1);
            for cc = 1:length(interps)
                kernVector(cc) = kernel(targetType,interps(cc,:),xSamp(mm,:),targetHyper);
            end
            target_Func_Orb(M+1) = coefs'*kernVector;
            M = M+1;
        end
    end
    ...
    functionEstimate = zeros(16490,1);
