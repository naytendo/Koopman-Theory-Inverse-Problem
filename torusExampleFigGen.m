...
meshRes = 0.1;
theta1Range = 0:meshRes:2*pi;
theta2Range = 0:meshRes:2*pi;
...
for ii = 1:size(targetFunc_Mesh,1)
    ...
    kernVector = zeros(length(interps),1);
        for cc = 1:length(interps)
            kernVector(cc) = kernel(type,interps(cc,:),[theta1Mesh(ii,jj),theta2Mesh(ii,jj)],beta);
        end
    targetFunc_Mesh(ii,jj) = coefs'*kernVector;
    end
end

surf(u_Mesh,v_Mesh,w_Mesh,targetFunc_Mesh,'EdgeColor','none','FaceAlpha',0.7);
colorbar
%%
n = 2;
